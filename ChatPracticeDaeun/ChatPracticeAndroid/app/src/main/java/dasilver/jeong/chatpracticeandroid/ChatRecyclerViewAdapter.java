...
public class ChatRecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public static final int VIEW_TYPE_YOUR_MESSAGE = 2;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (viewType == VIEW_TYPE_DATE) {
            View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_chat_recycler_date, parent, false);
            return new ChatRecyclerDateViewHolder(v);
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if (holder instanceof ChatRecyclerDateViewHolder) {
            ((ChatRecyclerDateViewHolder) holder).chatDateText.setText(chatRecyclerItems.get(position).dateText);

        } else if (holder instanceof ChatRecyclerMyMessageViewHolder) {
